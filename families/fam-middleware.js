...
exports.checkIfFamilyExists = async (req,res,next) => {
    try {
        const guest = await db('families').where({id: req.params.id})
        let errMessage = `Family with id of ${req.params.id} does not exist`
